...
const UploadForm = () => {
  ...
  const [lokasiList, setLokasiList] = useState<{ lokasi_id: number; nama_lokasi: string }[]>([]);
  const [titikLokasiList, setTitikLokasiList] = useState<{ id_titik_lokasi: number; nama_titik_lokasi: string }[]>([]);
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-gray-100 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <select name="jenis_sarana" value={formData.jenis_sarana} onChange={handleChange} required className="mb-2 p-2 w-full border rounded">
        ...
        {/* Dropdown Lokasi */}
        <select name="lokasi_id" value={formData.lokasi_id} onChange={handleChange} required className="mb-2 p-2 w-full border rounded">
          <option value="" disabled>Pilih Lokasi</option>
          {lokasiList.map((lokasi) => (
            <option key={lokasi.lokasi_id} value={lokasi.lokasi_id}>{lokasi.nama_lokasi}</option>
          ))}
        </select>

        {/* Dropdown Titik Lokasi */}
        <select name="id_titik_lokasi" value={formData.id_titik_lokasi} onChange={handleChange} required className="mb-2 p-2 w-full border rounded" disabled={!formData.lokasi_id}>
          <option value="" disabled>Pilih Titik Lokasi</option>
          {titikLokasiList.map((titik) => (
            <option key={titik.id_titik_lokasi} value={titik.id_titik_lokasi}>{titik.nama_titik_lokasi}</option>
          ))}
        </select>
        ...
        <div className="mb-2">
          ...
          <div className="flex gap-4">
            <label><input type="radio" name="status_pemasangan" value="Terpasang" onChange={handleChange} className="mr-2" /> Terpasang</label>
            <label><input type="radio" name="status_pemasangan" value="Belum Terpasang" onChange={handleChange} className="mr-2" /> Belum Terpasang</label>
          </div>
        </div>
